  ...
  def policy_eval(self, num_steps): # 方策評価 方策に対する状態価値関数を計算
    ...
    for i in range(num_steps):
      ...
      for s in self.states:
        ...
        # proc_action(s, a)としたら下でs is changedと出力される
        # if _s != s:
        #   print("s is changed")
        # else:
      ...
      for s in self.states:
        for a in Actions:
          next_state, reward = self.env.proc_action(copy.copy(s), a)
          self.action_val[s[0], s[1], a.value] = reward + self.gamma * self.state_val[next_state[0]][next_state[1]] # 練習問題4.1
...
#             temp += self.random_policy[next_state[0], next_state[1], next_action.value] * self.action_val[next_state[0]][next_state[1]][next_action.value]
...
#           copied_action_val[s[0]][s[1]][a.value] = new_action_val
#
...
#       for next_action in Actions:
#         temp += self.random_policy[next_state[0], next_state[1], next_action.value] * self.action_val[next_state[0]][next_state[1]][next_action.value]
#       exp_act_val = self.random_policy[s[0], s[1], a.value] * (reward + self.gamma * temp)
#
#       if max_act_val < exp_act_val:
#         max_act_val = exp_act_val
#         greedy_actions = [a.name]
